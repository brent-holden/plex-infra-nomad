job "backup-netbootxyz-config" {
  ...
  group "backup-netbootxyz" {
    ...
    volume "config" {
      type      = "host"
      source    = "[[ .app.netbootxyz.volumes.config.name ]]"
      read_only = true
    }
    ...
    task "unlock" {
      ...
      config {
        ...
        args = [
          "--repo", "rest:[[ .app.rclone_restic.service_url ]]:[[ .app.rclone_restic.ports.rclone ]]/netbootxyz/config",
          "unlock",
        ]
      }
